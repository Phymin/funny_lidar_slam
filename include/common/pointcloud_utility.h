...
template <typename PointT> inline float DistanceSquared(const PointT& pt1, const PointT& pt2)
{
...
PclPointToEigen<float, 3, pcl::PointXYZI>(const pcl::PointXYZI& pt)
{
    return pt.getVector3fMap();
}
...
inline PCLPointXYZ TransformPoint(const PCLPointXYZ& pt, const Mat3d& R, const Vec3d& t)
{
    const Vec3f temp = R.cast<float>() * Vec3f(pt.x, pt.y, pt.z) + t.cast<float>();
    return {temp.x(), temp.y(), temp.z()};
}

inline PCLPointXYZ TransformPoint(const PCLPointXYZ& pt, const Mat3f& R, const Vec3f& t)
{
    const Vec3f temp = R * Vec3f(pt.x, pt.y, pt.z) + t;
    return {temp.x(), temp.y(), temp.z()};
}

inline PCLPointXYZI TransformPoint(const PCLPointXYZI& pt, const Mat3d& R, const Vec3d& t)
{
    const Vec3f temp = R.cast<float>() * Vec3f(pt.x, pt.y, pt.z) + t.cast<float>();
...
inline PCLPointXYZI TransformPoint(const PCLPointXYZI& pt, const Mat3f& R, const Vec3f& t)
{
    const Vec3f temp = R * Vec3f(pt.x, pt.y, pt.z) + t;
    PCLPointXYZI pt_trans;
    pt_trans.x = temp.x();
...
inline PointXYZIRT TransformPoint(const PointXYZIRT& pt, const Mat3f& R, const Vec3f& t)
{
    const Vec3f temp = R * Vec3f(pt.x, pt.y, pt.z) + t;
...
inline PCLPointCloudXYZ TransformPointCloud(const PCLPointCloudXYZ& cloud, const Mat3f& R,
                                            const Vec3f& t)
{
...
inline PCLPointCloudXYZ TransformPointCloud(const PCLPointCloudXYZ& cloud, const Mat3d& R,
                                            const Vec3d& t)
{
    const unsigned int N = cloud.size();
...
inline PCLPointCloudXYZI TransformPointCloud(const PCLPointCloudXYZI& cloud, const Mat3f& R,
                                             const Vec3f& t)
{
    const unsigned int N = cloud.size();
...
inline PCLPointCloudXYZI TransformPointCloud(const PCLPointCloudXYZI& cloud, const Mat3d& R,
                                             const Vec3d& t)
{
    const unsigned int N = cloud.size();
...
inline PCLPointCloudXYZI::Ptr TransformPointCloud(const PCLPointCloudXYZI::Ptr& cloud,
                                                  const Mat4d& T)
{
    CHECK_NOTNULL(cloud);
    const unsigned int N = cloud->size();
...
inline PCLPointCloudXYZI::Ptr VoxelGridCloud(const PCLPointCloudXYZI::Ptr& cloud,
...
inline void RemoveNaNFromPointCloud(const pcl::PointCloud<PointT>& cloud_in,
