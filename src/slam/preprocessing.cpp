...
PreProcessing::PreProcessing(System* system_ptr) : system_ptr_(system_ptr)
{
    ...
    if (ConfigParameters::Instance().registration_and_searcher_mode_ == kLoamFull_KdTree)
    {
        cloud_projector_ptr_ = std::make_shared<loam::PointcloudProjector>(
            lidar_distortion_corrector_ptr_, LidarModel::Instance()->horizon_scan_num_,
            LidarModel::Instance()->vertical_scan_num_, LidarModel::Instance()->h_res_,
        ...
        feature_extractor_ptr_ = std::make_shared<loam::FeatureExtractor>(
            ...
            LidarModel::Instance()->horizon_scan_num_, LidarModel::Instance()->vertical_scan_num_);
    }
...
void PreProcessing::Run()
{
    ...
    while (rclcpp::ok())
    {
        sensor_msgs::msg::PointCloud2::SharedPtr raw_cloud_ros;
        ...
        auto cloud_start_timestamp =
            static_cast<uint64_t>(static_cast<int64_t>(raw_cloud_ptr->header.stamp) +
                                  static_cast<int64_t>(min_max_offset_time.first * 1.0e6));
        auto cloud_end_timestamp =
            static_cast<uint64_t>(static_cast<int64_t>(raw_cloud_ptr->header.stamp) +
        ...
        const auto imu_data_segment = system_ptr_->imu_data_searcher_ptr_->GetDataSegment(
        ...
        if (last_cloud_timestamp == std::numeric_limits<TimeStampUs>::max())
        {
            ...
            system_ptr_->imu_data_searcher_ptr_->SearchNearestTwoData(curr_cloud_timestamp, data_l,
                                                                      data_r);
        ...
        else
        {
            ...
            cloud_cluster_ptr->imu_data_ = system_ptr_->imu_data_searcher_ptr_->GetDataSegment(
            ...
            ConfigParameters::Instance().registration_and_searcher_mode_ == kIncrementalNDT)
        {
            ...
            for (unsigned int i = 0; i < cloud_cluster_ptr->raw_cloud_.size(); ++i)
            {
                ...
                if (!lidar_distortion_corrector_ptr_->ProcessPoint(pt_x, pt_y, pt_z, pt_x, pt_y,
                                                                   pt_z, relative_time))
                {
                    continue;
                }
...
PreProcessing::ConvertMessageToCloud(sensor_msgs::msg::PointCloud2::SharedPtr cloud_ros_ptr)
{
    ...
    else if (LidarModel::Instance()->lidar_sensor_type_ == LidarModel::LidarSensorType::LIVOX_AVIA)
    {
        ...
        for (size_t i = 0; i < point_size; ++i)
        {
                 ...
                 (cloud_livox.points[i].tag & 0x30) == 0x00))
            {
                ...
                point_xyzirt.time =
                    static_cast<float>(static_cast<double>(cloud_livox.points[i].time) *
                                       ConfigParameters::Instance().lidar_point_time_scale_);
    ...
    else if (LidarModel::Instance()->lidar_sensor_type_ == LidarModel::LidarSensorType::None)
    {
        ...
        for (size_t i = 0; i < point_size; ++i)
        {
            ...
            float xy =
                std::sqrt(cloud_none[i].x * cloud_none[i].x + cloud_none[i].y * cloud_none[i].y);
...
void PreProcessing::ComputePointOffsetTime(const pcl::PointCloud<PointXYZIRT>::Ptr& cloud,
                                           const double lidar_rate)
{
    ...
    for (size_t i = 0; i < cloud_size; i++)
    {
        ...
        {
            cloud->points[i].time =
                static_cast<float>((yaw_first_scan[ring] - yaw + 2.0 * M_PI) / lidar_omega);
        }
...
PreProcessing::GetLidarPointMinMaxOffsetTime(const pcl::PointCloud<PointXYZIRT>::Ptr& cloud)
{
